...
public class Rsi {
	...
	public BigDecimal getRsi_1DAY(Bitbankcc bb,CurrencyPair cp,int num) throws Exception{
		String strYYYYMMDD = DateAndTime.getBaseDate();
		List<Candlestick.Ohlcvs.Ohlcv>  cs = candlestickLogic.getCs(bb,cp,CandleType._1DAY,strYYYYMMDD.substring(0, 4),num+1);
		return getRsi(cs,num);
	}
	...
	public BigDecimal getRsi_1HOUR(Bitbankcc bb,CurrencyPair cp,int num) throws Exception{
		String strYYYYMMDD = DateAndTime.getBaseDate();
		List<Candlestick.Ohlcvs.Ohlcv>  cs = candlestickLogic.getCs(bb, cp, CandleType._1HOUR, strYYYYMMDD,num+1);
		return getRsi(cs,num);
	}
	...
	public BigDecimal getRsi_15MIN(Bitbankcc bb,CurrencyPair cp, int num) throws Exception{
		String strYYYYMMDD = DateAndTime.getBaseDate();
		List<Candlestick.Ohlcvs.Ohlcv>  cs = candlestickLogic.getCs(bb, cp, CandleType._15MIN, strYYYYMMDD,num+1);
		return getRsi(cs,num);
	}
	...
	public BigDecimal getRsi_5MIN(Bitbankcc bb,CurrencyPair cp,int num) throws Exception{
		String strYYYYMMDD = DateAndTime.getBaseDate();
		List<Candlestick.Ohlcvs.Ohlcv>  cs = candlestickLogic.getCs(bb, cp, CandleType._5MIN, strYYYYMMDD,num+1);
		return getRsi(cs,num);
	}
	...
	public BigDecimal getRsi_1MIN(Bitbankcc bb,CurrencyPair cp , int num) throws Exception{
		String strYYYYMMDD = DateAndTime.getBaseDate();
		List<Candlestick.Ohlcvs.Ohlcv>  cs = candlestickLogic.getCs(bb, cp, CandleType._1MIN, strYYYYMMDD,num + 1);
		return getRsi(cs,num);
	}
...
//        bc15Day[14]=BigDecimal.valueOf(1100);
	...
	public BigDecimal getRsi(List<Candlestick.Ohlcvs.Ohlcv>  cs , int num){
        // 15日間の値を取得して、配列に格納
        int count = 0;
        BigDecimal bc15Day[] = new BigDecimal[num + 1];
         // 金額の取得
        for(int i=-num;i<=0;i++) {
            	bc15Day[count]=cs.get(cs.size()+i-1).close;
            	//System.out.println(cs.get(cs.size()+i-1).date + " " + bc15Day[count]);
...
//        bc15Day[14]=BigDecimal.valueOf(1100);
        ...
        plus = plus.divide(BigDecimal.valueOf(num), 4 ,BigDecimal.ROUND_HALF_UP);
        minus = minus.divide(BigDecimal.valueOf(num), 3 ,BigDecimal.ROUND_HALF_UP);
        //System.out.println("plus:" + plus + " minus:" + minus);
        minus = plus.add(minus.abs());
        //System.out.println("plus:" + plus + " minus:" + minus);
